...
export default function ResultPage() {
  ...
  return (
    <div className="min-h-screen p-6">
      <div className="max-w-4xl mx-auto bg-[#3a3b44] p-6 rounded border border-[#4a4b52]">
        <h1 className="text-2xl font-bold mb-2">Quiz Result</h1>
        <div className="mb-4 text-[#E6E7E9]">
        ...
        <div className="mb-4">
          ...
          {wrongs.length === 0 ? (
            <div className="text-[#7ee787]">All correct — great job!</div>
          ) : (
            <div className="space-y-4">
              {wrongs.map(r => {
                const q = QUESTIONS.find(x => x.id === r.questionId) as Q;
                return (
                  <div key={q.id} className="p-3 rounded border border-[#4a4b52] bg-[#2f3035]">
                    <div className="font-medium mb-1 text-[#E6E7E9]">{q.text}</div>
                    {q.image && <img src={q.image} alt={q.id} className="mb-2 max-w-full rounded border border-neutral-600" />}
                    <div className="text-sm text-[#bfc3c6]">Your answer: <span className="font-semibold text-[#E6E7E9]">{r.choiceIndex === null ? "No answer" : q.choices[r.choiceIndex]}</span></div>
                    <div className="text-sm text-[#bfc3c6]">Correct answer: <span className="font-semibold text-[#E6E7E9]">{q.choices[q.answerIndex]}</span></div>
                  </div>
                );
              })}
            </div>
        ...
        <div className="mt-4">
          <button className="px-4 py-2 bg-[#7c93ff] text-black rounded font-semibold" onClick={() => { localStorage.removeItem("quiz_answers"); localStorage.removeItem("quiz_final"); window.location.href = "/"; }}>
            Back to Home
          </button>
        </div>
      </div>
